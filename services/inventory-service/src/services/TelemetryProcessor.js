  ...
  async _evaluateThresholds(sensorId, type, value, unit) {
    if (type === 'weight') {
      if (value < 10 && unit === 'kg') {
        ...
        await AlertPublisher.publishAlert('InventoryLow', `alert-low-stock-${sensorId}`, payload);
      }
      ...
      if (value > 6 && unit === 'C') {
        ...
        await AlertPublisher.publishAlert('TemperatureAlert', `alert-temp-${sensorId}`, payload);
      }
    }
  }
}
