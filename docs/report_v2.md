...
4. [Kiến Trúc Hệ Thống](#4-kiến-trúc-hệ-thống)
   ...
   - 4.2 [Architecture Decision Records](#42-architecture-decision-records-adr)
...
Phần này đánh giá ba phong cách kiến trúc phổ biến — Layered, Service-based và Microservices — thông qua lăng kính của các architecture characteristics đã được xác định ở Chương 3. Phương pháp đánh giá dựa trên hai chiều: (1) **phân tích lý thuyết** về nguyên lý thiết kế và đặc tính kỹ thuật của từng kiến trúc, và (2) **ánh xạ vào bài toán cụ thể** của IRMS với các NFR đã đề ra.
...
Layered Architecture (hay N-tier Architecture) tổ chức hệ thống thành các tầng nằm ngang, trong đó mỗi tầng chỉ được phép phụ thuộc vào tầng liền kề bên dưới — nguyên tắc này được gọi là *strict layer dependency*. Mô hình chuẩn gồm bốn tầng: Presentation (xử lý HTTP request, render UI), Business Logic (nghiệp vụ), Data Access (truy vấn database), và Database. Toàn bộ luồng xử lý là đồng bộ và tuần tự — một request đi từ trên xuống và response trả về ngược lại.

Triết lý thiết kế của Layered Architecture là *separation of concerns theo chiều ngang* — mỗi tầng có một trách nhiệm kỹ thuật rõ ràng và có thể được thay thế độc lập. Đây là pattern phổ biến nhất trong các ứng dụng enterprise truyền thống và thường được áp dụng trong các hệ thống CRUD quy mô nhỏ đến trung bình.

```mermaid
flowchart TD
    A[Presentation Layer\nHTTP · REST · UI Rendering]
    B[Business Logic Layer\nValidation · Domain Rules · Orchestration]
    C[Data Access Layer\nORM · Query Builder · Transactions]
    D[(Shared Database\nSingle Schema)]
    A --> B --> C --> D
```
...
Đơn vị triển khai của Layered Architecture là toàn bộ ứng dụng — tất cả các tầng được đóng gói và deploy cùng nhau như một artifact duy nhất (monolith). Điều này dẫn đến *deployment coupling*: một thay đổi nhỏ ở bất kỳ tầng nào cũng yêu cầu build và deploy lại toàn bộ hệ thống. Giao tiếp giữa các tầng là *in-process function call* — không có network overhead, nhưng cũng không có ranh giới fault isolation. Khi một tầng gặp lỗi (ví dụ: Business Logic bị exception không xử lý), lỗi đó lan truyền lên Presentation Layer và có thể khiến toàn bộ process crash.

**Đánh giá theo yêu cầu IRMS**

Xét theo tiêu chí *Real-Time Responsiveness* (NFR2: latency < 1 giây, NFR4: push đến KDS): Layered Architecture hoạt động theo mô hình request-response đồng bộ thuần túy. Để đạt real-time, hệ thống phải dùng long-polling hoặc tích hợp WebSocket như một layer đặc biệt — điều này phá vỡ mô hình strict layering và làm tăng complexity mà không có lợi ích kiến trúc rõ ràng. Với yêu cầu IRMS là push event từ Ordering sang Kitchen trong dưới 500ms mà không tốn tài nguyên polling, kiến trúc này không phù hợp về mặt cơ bản.

Xét theo *Scalability* (NFR1: 100+ orders/min, NFR6: scale per service): Đơn vị scale duy nhất của Layered Architecture là toàn bộ ứng dụng. Trong giờ cao điểm, nếu Ordering Service là bottleneck, hệ thống buộc phải scale cả Business Logic lẫn Data Access lên cùng lúc — lãng phí tài nguyên và không giải quyết được gốc rễ vấn đề. Hơn nữa, shared database là điểm nghẽn cố hữu không thể giải quyết bằng horizontal scaling ở tầng application.

Xét theo *Fault Tolerance* (NFR3, NFR7): Không có fault boundary giữa các domain. Nếu module xử lý IoT sensor gặp vấn đề (ví dụ: sensor gửi malformed data làm exception), lỗi này có thể ảnh hưởng đến toàn bộ process, bao gồm cả luồng đặt món. Đây là vi phạm nghiêm trọng với một hệ thống yêu cầu 99.9% uptime cho luồng order.

**Kết luận**: Layered Architecture phù hợp cho ứng dụng CRUD với traffic ổn định và không có yêu cầu real-time. Với IRMS, kiến trúc này không đáp ứng NFR1, NFR2, NFR3, NFR6, NFR7.
...
Service-based Architecture là bước trung gian giữa Layered (monolith) và Microservices, được Mark Richards mô tả trong *Software Architecture Patterns* như một kiến trúc *coarse-grained service decomposition*. Hệ thống được phân tách thành 4–12 service độc lập về mặt deployment, mỗi service đại diện cho một domain nghiệp vụ lớn (ví dụ: User Service, Order Service, Analytics Service). Điểm đặc trưng quan trọng nhất phân biệt với Microservices: các service **chia sẻ chung một database** — chúng truy cập vào cùng một schema, không có ranh giới data ownership.

Mô hình này kế thừa tư tưởng của Service-Oriented Architecture (SOA) nhưng đơn giản hơn, không yêu cầu Enterprise Service Bus (ESB) hay WS-* protocols. Giao tiếp giữa các service thường là synchronous REST/RPC.
...
flowchart TD
    ...
    OS & KS & AS --> DB[(Shared Relational Database\nShared Schema · Shared Connection Pool)]
```

**Đặc điểm kỹ thuật**

Service-based Architecture đạt được *deployment independence* ở tầng application — mỗi service có thể được build và deploy riêng lẻ mà không ảnh hưởng đến các service khác. Tuy nhiên, kiến trúc này vẫn duy trì *data coupling* tại tầng database. Khi cần migration schema (thêm cột, đổi kiểu dữ liệu), tất cả các service cùng truy cập vào table đó đều phải được kiểm tra và có thể cần deploy đồng thời — điều này tạo ra *implicit coupling* mà không dễ phát hiện ở giai đoạn thiết kế.

Về fault isolation: mặc dù service process được cô lập, shared database là *single point of failure* cho tầng data. Nếu database quá tải hoặc có deadlock, tất cả service đều bị ảnh hưởng đồng thời.

**Đánh giá theo yêu cầu IRMS**

Xét theo *Real-Time Responsiveness*: Service-based Architecture cải thiện so với Layered nhờ services có thể chuyên biệt hóa, nhưng vẫn phụ thuộc vào shared database làm trung gian. Để Kitchen Service nhận được OrderPlaced event, cơ chế phổ biến nhất là database polling hoặc DB trigger — cả hai đều có latency không ổn định và không đáp ứng yêu cầu < 500ms một cách tin cậy. WebSocket có thể tích hợp được nhưng không phải native pattern của kiến trúc này.

Xét theo *Scalability và Technology Diversity*: Đây là hạn chế quan trọng nhất của Service-based Architecture đối với IRMS. Database chung buộc tất cả service phải dùng cùng một engine — không thể deploy InfluxDB cho Inventory Service (time-series) trong khi vẫn dùng PostgreSQL cho Ordering. IRMS có yêu cầu polyglot persistence rõ ràng: time-series cho sensor, relational cho orders, key-value cho real-time metrics. Shared database là một ràng buộc cứng làm triệt tiêu khả năng này.

Xét theo *Fault Tolerance cho IoT* (NFR7): IoT sensor có thể gửi data burst (nhiều reading liên tục) hoặc gửi malformed payload. Nếu Inventory Service bị overload bởi sensor data, shared connection pool của database sẽ bị tiêu thụ, ảnh hưởng trực tiếp đến Ordering Service và Kitchen Service — vi phạm fault isolation.

**Kết luận**: Service-based Architecture giảm deployment coupling so với Layered nhưng không giải quyết được data coupling — đây chính là điểm nghẽn cốt lõi của IRMS với yêu cầu polyglot persistence và fault isolation tại tầng data. Kiến trúc này không đáp ứng NFR2, NFR6, NFR7.
...
Microservices Architecture phân rã hệ thống thành các service *fine-grained*, mỗi service sở hữu một *Bounded Context* theo nguyên lý Domain-Driven Design (DDD) — tức là service không chỉ độc lập về mặt code và deployment mà còn độc lập hoàn toàn về *data ownership*. Mỗi service có database riêng (Database-per-Service Pattern) và các service không được phép truy cập trực tiếp vào database của nhau. Giao tiếp giữa các service được thực hiện thông qua hai cơ chế: synchronous REST/gRPC cho các truy vấn cần kết quả tức thì, và asynchronous event streaming (Kafka, RabbitMQ) cho các luồng domain events.

Một đặc điểm quan trọng khác là *polyglot persistence* — mỗi service được tự do chọn database engine phù hợp nhất với workload của mình, và *polyglot programming* — mỗi service có thể dùng ngôn ngữ/framework khác nhau. Kiến trúc này được thiết kế đặc biệt cho các hệ thống large-scale cần scale độc lập theo từng domain.

```mermaid
flowchart TD
    GW[API Gateway\nNGINX · JWT · Rate-limit]
    GW --> OS[Ordering Service]
    GW --> KS[Kitchen Service]
    ...
    AN --> DB4[(Redis + PostgreSQL\nmetrics · reports)]
    OS -->|"OrderPlaced event"| KB[Kafka Event Bus]
    KB -->|"subscribe"| KS & IS & AN
```

**Đặc điểm kỹ thuật**

Microservices đạt *complete runtime isolation* — mỗi service là một process độc lập với memory space, connection pool, và failure domain riêng. Fault boundary ở đây không chỉ là service process mà còn là database: một service quá tải sẽ không ảnh hưởng đến connection pool của service khác. Tuy nhiên, distributed system đi kèm với những thách thức đặc thù: *network latency* giữa các service (thay vì in-process call), *partial failure* (một service có thể fail trong khi các service khác vẫn chạy), và *eventual consistency* khi dữ liệu được đồng bộ qua event stream thay vì transaction ACID xuyên service.

**Đánh giá theo yêu cầu IRMS**

Xét theo *Real-Time Responsiveness* (NFR2): Event-driven thông qua Kafka cho phép Ordering Service publish `OrderPlaced` event và Kitchen Service nhận event đó trong 10–50ms. Không cần polling, không cần shared DB trigger. Socket.io được tích hợp tự nhiên vào Kitchen Service để push task xuống KDS Display ngay sau khi event được consume. Latency end-to-end Ordering → KDS đo được dưới 450ms, đáp ứng NFR2.

Xét theo *Scalability* (NFR1, NFR6): Ordering Service có thể scale từ 2 lên 5 instance trong lunch rush mà hoàn toàn không ảnh hưởng đến Kitchen Service hay Analytics Service. Database-per-Service loại bỏ database bottleneck — mỗi service scale cả application layer lẫn database layer độc lập. Polyglot persistence cho phép dùng đúng engine cho đúng workload: InfluxDB xử lý time-series sensor data với write throughput hàng nghìn points/giây mà PostgreSQL không thể đạt được hiệu quả tương đương.

Xét theo *Fault Tolerance* (NFR3, NFR7): Nếu Kitchen Service crash, Ordering Service vẫn tiếp tục nhận order bình thường. Kafka buffer toàn bộ `OrderPlaced` events trong thời gian Kitchen offline. Khi Kitchen restart, nó consume lại từ offset cuối cùng — không mất order nào. Tương tự, nếu IoT sensor flood data làm Inventory Service quá tải, Ordering và Kitchen hoàn toàn không bị ảnh hưởng do tách biệt fault domain.

Xét theo *Trade-off*: Distributed system đòi hỏi operational maturity cao hơn — cần distributed tracing để debug, cần health check cho từng service, và cần xử lý eventual consistency trong các luồng cross-service. Trong phạm vi đồ án, nhóm chấp nhận trade-off này vì các lợi ích về fault isolation, scalability và technology diversity vượt trội so với complexity tăng thêm.

Trong ba kiến trúc được xem xét, Microservices là lựa chọn duy nhất đáp ứng đầy đủ các NFR ở mức ưu tiên P0 và P1 của IRMS. Layered Architecture chỉ phù hợp với bài toán CRUD đơn giản, không có real-time hay IoT. Service-based Architecture tiến thêm một bước về deployment independence nhưng vẫn bị khóa bởi shared database — điều kiện cần thiết để đồng thời sử dụng InfluxDB cho sensor time-series, PostgreSQL cho orders và Redis cho real-time metrics là không thể đạt được trong mô hình này. Phân tích trên là cơ sở trực tiếp để nhóm hình thành các quyết định kiến trúc cụ thể, được ghi lại chính thức trong mục 4.2.

---

### 4.2 Architecture Decision Records (ADR)

Mỗi quyết định dưới đây xuất phát từ một vấn đề cụ thể được phát hiện trong quá trình phân tích ở mục 4.1. Format ADR (Architecture Decision Record) được dùng để ghi lại không chỉ *cái gì* được quyết định mà còn *tại sao*, và *hệ quả nào* được chấp nhận — giúp tài liệu kiến trúc có thể truy vết và lý giải được theo thời gian.

---

#### ADR-001: Microservices + Event-Driven Architecture

**Trạng thái**: Accepted

**Bối cảnh**: IRMS cần xử lý đồng thời ba luồng có tính chất khác nhau — order flow đòi hỏi real-time dưới 1 giây, IoT telemetry liên tục từ 50–100 sensor, và analytics aggregate không được blocking luồng chính. Layered Architecture không đáp ứng real-time do mô hình request-response đồng bộ; Service-based Architecture bị bottleneck tại shared database khi cần dùng InfluxDB cho sensor time-series song song với PostgreSQL cho order.

**Quyết định**: Chọn **Microservices + Event-Driven Architecture** với 7 service hoàn toàn độc lập và Kafka làm event bus trung tâm. Mỗi service có database riêng, Dockerfile riêng, scale riêng và không import code của nhau.

**Hệ quả**: Fault isolation được đảm bảo — Kitchen Service crash không ảnh hưởng Ordering Service vì events đã được buffer trong Kafka. Ordering Service có thể scale ×5 trong lunch rush mà không kéo theo Kitchen hay Analytics. Mỗi service được tự do chọn database engine phù hợp nhất với workload của mình. Đánh đổi chấp nhận là operational complexity tăng (distributed tracing, partial failure, eventual consistency) — tuy nhiên đây là bài toán đồ án nên prototype không cần production-grade observability stack.
...
**Bối cảnh**: Trong kiến trúc Microservices, nếu các service chia sẻ chung một database thì coupling tầng data sẽ phá vỡ tính độc lập — một migration schema có thể làm hỏng nhiều service khác cùng lúc. Ngoài ra, mỗi domain trong IRMS có đặc thù lưu trữ rất khác nhau: order cần ACID transaction, sensor telemetry cần write throughput cao và query theo time window, metrics dashboard cần đọc sub-millisecond.

**Quyết định**: Mỗi service sở hữu database riêng và dữ liệu chỉ được truy cập thông qua API của service đó, không có service nào query trực tiếp vào database của service khác. Cụ thể: Ordering, Kitchen và Auth dùng **PostgreSQL** (ACID, relational); Inventory dùng **InfluxDB** (time-series, write-heavy); Analytics dùng **Redis** cho real-time counter và **PostgreSQL** cho report storage lâu dài.

**Hệ quả**: Schema thay đổi trong một service không ảnh hưởng bất kỳ service nào khác. Mỗi service chọn được database engine tối ưu nhất cho workload của mình. Đánh đổi chấp nhận là không thể có JOIN xuyên service — dữ liệu cần aggregate phải đi qua event hoặc API call, làm cross-service query phức tạp hơn. Trong phạm vi đồ án, đây không phải điểm nghẽn vì các báo cáo analytics được xây dựng từ event stream Kafka thay vì JOIN trực tiếp.

---

#### ADR-003: Apache Kafka làm Event Bus (thay vì REST polling hoặc RabbitMQ)

**Trạng thái**: Accepted

**Bối cảnh**: Luồng cốt lõi `OrderPlaced` cần truyền từ Ordering Service đến Kitchen Service trong dưới 500ms và phải đảm bảo không mất event ngay cả khi Kitchen tạm thời offline để restart. REST polling không phù hợp vì latency cao và tốn tài nguyên khi poll liên tục. RabbitMQ là message queue đơn giản hơn nhưng không hỗ trợ event replay — khi consumer offline thì event bị mất.

**Quyết định**: Dùng **Apache Kafka** làm event bus với các topic được phân tách rõ theo domain: `orders` (Ordering → Kitchen/Analytics), `kitchen_completed` (Kitchen → Analytics), `alerts` (Inventory → Notification), `sensor.telemetry` (IoT → Inventory), `payment-requests` và `payment-completed`. Kafka lưu event theo retention period, cho phép consumer restart và consume lại từ offset cuối cùng mà không mất dữ liệu.

**Hệ quả**: Guaranteed delivery được đảm bảo — Kitchen Service restart và consume lại từ offset, không mất order nào. Multiple consumers có thể cùng subscribe một topic mà không cần Ordering Service biết: Analytics subscribe `orders` để tính metrics song song với Kitchen. Latency từ publish đến consume dưới 50ms. Đánh đổi chấp nhận là Kafka cùng Zookeeper tiêu thụ khoảng 512MB RAM — đáng kể trên VPS 8GB — và at-least-once delivery đòi hỏi consumer phải idempotent. Trong prototype, điều này được xử lý đơn giản bằng consumer group offset commit.

---

#### ADR-004: VPS + Docker Compose (thay vì Kubernetes hoặc Cloud-managed)

**Trạng thái**: Accepted

**Bối cảnh**: Hệ thống cần một môi trường deploy đơn giản, đủ để chạy toàn bộ 7 service cùng infrastructure trong điều kiện của một nhà hàng đơn lẻ. Kubernetes có learning curve cao, đòi hỏi kiến thức vận hành phức tạp và overkill cho quy mô này. AWS EKS hay GKE có chi phí cluster control plane liên tục ngay cả khi hệ thống idle.

**Quyết định**: Triển khai toàn bộ hệ thống trên **một VPS duy nhất (4 vCPU, 8 GB RAM)** với **Docker Compose** làm orchestrator. NGINX chạy trực tiếp trên OS của VPS đóng vai trò reverse proxy và TLS termination. CI/CD pipeline sử dụng GitHub Actions để build image, push lên Docker Hub và SSH vào VPS để pull image mới và restart container.

**Hệ quả**: Toàn bộ môi trường khởi động bằng một lệnh `docker-compose up --build` trong dưới 2 phút — thuận tiện cho cả development lẫn demo. Không cần quản lý cluster hay control plane. Chi phí ước tính 20–40 USD/tháng so với 200+ USD/tháng cho Kubernetes cluster tương đương. Đánh đổi chấp nhận là không có auto-scaling tự động và VPS là single point of failure — được giảm nhẹ bằng restart policy `on-failure` và backup định kỳ, đủ cho yêu cầu đồ án. Path migrate lên Docker Swarm hoặc Kubernetes về sau không đòi hỏi thay đổi code, chỉ cần cập nhật orchestration config.
...
`verifyToken` và `verifyRole(roles)` đều tuân thủ cùng signature `(req, res, next)` — đây chính là contract của Express middleware. Cả hai có thể compose tùy ý trong chuỗi (`verifyToken → verifyRole(['CHEF', 'MANAGER']) → handler`) mà không phá vỡ hành vi. Thêm middleware mới vào chain chỉ cần đảm bảo cùng signature — không cần sửa các middleware đã có.
...
Cách các nguyên lý trên được hiện thực cụ thể trong code — bao gồm ví dụ vi phạm và ví dụ áp dụng thực tế từng service — được trình bày chi tiết tại **Mục 8.3**.
...
Mỗi mục dưới đây trình bày theo cấu trúc: **Vi phạm** (code xấu và hệ quả) → **Tuân thủ trong IRMS** (code thực tế từ codebase) → **Phân tích** (tại sao thiết kế như vậy giải quyết vấn đề).

---

#### 8.3.1 SRP — Ordering Service: Tách tầng trách nhiệm

**Vi phạm — "God Function"**: Controller xử lý HTTP, SQL và Kafka trong một function duy nhất.

```javascript
// ❌ Vi phạm SRP — ba lý do thay đổi trong một function
async function placeOrder(req, res) {
  const { tableId, items } = req.body;                        // HTTP concern
  if (!tableId || !items) return res.status(400).json({ error: 'Invalid' });

  let totalPrice = 0;
  for (const item of items) {
    const row = await pool.query(                             // DB concern trộn vào
      'SELECT * FROM menu_items WHERE id = $1', [item.menuItemId]);
    if (!row.rows[0].is_available) return res.status(400).json({ error: 'Unavailable' });
    totalPrice += row.rows[0].price * item.quantity;
  }
  const order = await pool.query(
    'INSERT INTO orders (table_id, total_price) VALUES ($1, $2) RETURNING *',
    [tableId, totalPrice]);

  await producer.send({                                       // Kafka concern trộn vào
    topic: 'orders',
    messages: [{ key: `order-${order.rows[0].id}`, value: JSON.stringify(order.rows[0]) }]
  });
  res.json(order.rows[0]);
}
// Hệ quả: đổi schema DB → sửa function này; đổi Kafka topic → sửa function này;
// đổi response format → sửa function này. Ba actor, một nơi thay đổi → merge conflict liên tục.
```

**Tuân thủ trong IRMS** — `ordering-service/src/services/OrderPlacementService.js`:

```javascript
// ✅ Mỗi class có đúng một lý do thay đổi
class OrderPlacementService {
  async placeOrder(userId, tableId, items) {
    // Orchestrate flow — không biết SQL, không biết Kafka chi tiết
  ...
  async _validateAndCalculate(items) {
    // Business logic thuần — delegate DB access xuống repository
    let totalPrice = 0;
  ...
  async _publishOrderPlaced(order, tableId, validatedItems) {
    // Kafka concern được đóng gói — OrderPlacementService không biết topic name
    const orderEvent = {
      orderId: order.id, tableId, items: validatedItems,
      total_price: order.total_price, timestamp: new Date().toISOString()
    };
...
**Phân tích**: Bốn lý do thay đổi được tách thành bốn đơn vị riêng: `OrderController` (HTTP format), `OrderPlacementService` (business flow), `OrderRepository` (SQL schema), `KafkaPublisher` (event format). Khi PostgreSQL schema thêm cột mới, chỉ sửa `orderRepository.js`. Khi Kafka topic đổi tên, chỉ sửa `_publishOrderPlaced`. `OrderPlacementService` không bị ảnh hưởng bởi cả hai thay đổi đó.

---

#### 8.3.2 OCP — Notification Service: Mở rộng không sửa đổi

**Vi phạm — "If/Else chain"**: Mỗi kênh thông báo mới đều phải mở `NotificationDispatcher` ra sửa.

```javascript
// ❌ Vi phạm OCP — thêm Telegram phải mở file này và thêm else if
class NotificationDispatcher {
  async dispatch(type, recipient, payload) {
    if (type === 'SMS') {
      console.log(`[SMS] Twilio → ${recipient}`);           // Twilio logic
    } else if (type === 'PUSH') {
      console.log(`[PUSH] FCM → ${recipient}`);             // FCM logic
    }
    // Thêm 'TELEGRAM'? → mở file, thêm else if → nguy cơ break SMS/PUSH logic cũ
  }
}
```

**Tuân thủ trong IRMS** — `notification-service/src/channels/`:

```javascript
// ✅ Base class mô tả contract — đóng để sửa
class INotificationChannel {
  async send(recipient, payload) { throw new Error('send() must be implemented'); }
}

// Mỗi kênh là extension riêng — thêm kênh = thêm file mới, không đụng file cũ
class SmsChannel extends INotificationChannel {
  async send(phone, payload) {
    console.log(`[SmsChannel] Twilio → ${phone}:`, payload); // mock: Twilio SDK
  }
}

class PushChannel extends INotificationChannel {
  async send(deviceToken, payload) {
    console.log(`[PushChannel] FCM → ${deviceToken}:`, payload); // mock: Firebase FCM
  }
}

// Dispatcher hoàn toàn không thay đổi khi thêm kênh mới
class NotificationDispatcher {
  constructor(channels) { this.channels = channels; } // nhận [SmsChannel, PushChannel, ...]
...
// Thêm TelegramChannel: tạo file TelegramChannel.js, extend INotificationChannel,
// truyền vào constructor — NotificationDispatcher không biết TelegramChannel tồn tại.
```

**Phân tích**: OCP được thể hiện ở hai cấp độ. Cấp class: `NotificationDispatcher` không bao giờ cần sửa khi thêm kênh mới — nó "đóng" với modification. Cấp kiến trúc: khi thêm `ReportingService` cần subscribe event `orders`, không cần sửa `OrderingService` — chỉ thêm consumer group mới. Kafka event bus chính là OCP ở tầng kiến trúc: producer đóng với modification, behavior của hệ thống mở rộng qua consumer mới.

---

#### 8.3.3 LSP — Auth Middleware: Contract nhất quán trong middleware chain

**Vi phạm — Middleware phá vỡ contract**: Middleware không tuân thủ signature `(req, res, next)` gây request bị treo hoặc crash.

```javascript
// ❌ Vi phạm LSP — thắt chặt postcondition: không gọi next() khi pass
const verifyAdminSpecial = (req, res) => {          // ← thiếu `next` parameter
  if (req.user.role !== 'MANAGER') {
    throw new Error('Forbidden');                    // ← ném exception thay vì res.json()
  }
  // Nếu role hợp lệ: không có next() → request bị treo vĩnh viễn
};
// router.get('/admin', verifyToken, verifyAdminSpecial, handler)
// → verifyAdminSpecial pass nhưng handler không bao giờ được gọi
```

**Tuân thủ trong IRMS** — `auth-service/src/middlewares/authMiddleware.js`:

```javascript
// ✅ Cả hai middleware đều tuân thủ contract (req, res, next) — có thể swap tự do
const verifyToken = (req, res, next) => {
  const authHeader = req.headers.authorization;
  if (!authHeader) return res.status(401).json({ message: 'Not authenticated' }); // fail fast
  const token = authHeader.split(' ')[1];
  jwt.verify(token, process.env.JWT_SECRET, (err, user) => {
    if (err) return res.status(403).json({ message: 'Invalid token' });  // fail fast
    req.user = user;
    next();  // ← luôn gọi next() khi pass — không treo request
  });
};

// verifyRole trả về middleware — cùng contract (req, res, next)
const verifyRole = (roles) => (req, res, next) => {
  if (!req.user || !roles.includes(req.user.role))
    return res.status(403).json({ message: 'Permission denied' }); // fail fast
  next();  // ← luôn gọi next() khi pass
};

// Compose tự do trong bất kỳ route nào:
// router.get('/tasks',   verifyToken, verifyRole(['CHEF', 'MANAGER']), getActiveTasks);
// router.post('/orders', verifyToken, verifyRole(['WAITER', 'CUSTOMER']), placeOrder);
// router.get('/reports', verifyToken, verifyRole(['MANAGER']), getReports);
```

**Phân tích**: LSP ở đây áp dụng cho function contract thay vì class hierarchy. Mọi middleware trong hệ thống đều tuân thủ một precondition (nhận `req, res, next`) và một postcondition (gọi `next()` khi pass, gọi `res.json()` khi fail — không bao giờ treo request). Repository pattern tuân thủ cùng nguyên lý: `PostgresOrderRepository` và `InMemoryOrderRepository` expose cùng interface, `OrderPlacementService` không phân biệt — swap trong test mà không thay đổi một dòng business logic.

---

#### 8.3.4 ISP — IoT Gateway: Interface chuyên biệt theo khả năng

**Vi phạm — "Fat Interface"**: Gộp tất cả capability vào một interface khiến sensor đơn giản phải implement method không có nghĩa.

```javascript
// ❌ Vi phạm ISP — fat interface ép BasicLoadCell implement thứ nó không có
class ISensor {
  ...
  async isOnline()        { throw new Error('Not implemented'); } // ← load cell không có heartbeat
  getLastHeartbeat()      { throw new Error('Not implemented'); } // ← load cell không có heartbeat
}

class BasicLoadCell extends ISensor {
  async readData()   { return { type: 'weight', value: 2.3, unit: 'kg' }; }
  getId()            { return this.cellId; }
  getType()          { return 'LOAD_CELL'; }
  async isOnline()   { throw new Error('Load cell has no heartbeat!'); } // ← buộc implement
  getLastHeartbeat() { return null; }  // ← trả null: misleading, dễ gây bug trong HealthMonitor
}
```

**Tuân thủ trong IRMS** — `inventory-service/src/sensors/`:

```javascript
// ✅ Hai interface nhỏ, chuyên biệt theo khả năng thực tế của từng sensor
class IReadableSensor  { async readData() {} getId() {} getType() {} }
class IHealthCheckable { async isOnline() {} getLastHeartbeat() {} }

// TemperatureSensor có cả hai khả năng → implement cả hai
class TemperatureSensor extends IReadableSensor {
  ...
  async isOnline() { return this.lastPing > Date.now() - 60000; } // có heartbeat
}

// BasicLoadCell chỉ có khả năng đọc → chỉ implement IReadableSensor
class BasicLoadCell extends IReadableSensor {
  async readData() { return { type: 'weight', value: 2.3, unit: 'kg' }; }
  getId()   { return this.cellId; }
  getType() { return 'LOAD_CELL'; }
  // Không có isOnline() — không bị ép implement method vô nghĩa
}

// Mỗi consumer phụ thuộc đúng interface nó cần — không thừa, không thiếu
...
**Phân tích**: `HealthMonitor` chỉ nhận `IHealthCheckable[]` — `BasicLoadCell` không bao giờ lọt vào danh sách đó vì nó không implement interface này. Không có null check, không có `if (sensor.isOnline !== undefined)` guard. Thêm `CO2Sensor` (chỉ cần đọc, không có heartbeat): tạo class extend `IReadableSensor`, `SensorPoller` tự động poll nó, `HealthMonitor` không biết nó tồn tại — không cần sửa một dòng code hiện có.

---

#### 8.3.5 DIP — Inventory & Analytics: Dependency flow từ infrastructure lên business

**Vi phạm — "Hardcoded Infrastructure"**: Business logic biết chi tiết database và message broker cụ thể.

```javascript
// ❌ Vi phạm DIP — TelemetryProcessor biết InfluxDB và Kafka chi tiết
const { InfluxDB, Point } = require('@influxdata/influxdb-client'); // ← hardcode InfluxDB
const { Kafka } = require('kafkajs');

class TelemetryProcessor {
  constructor() {
    this.influx = new InfluxDB({ url: process.env.INFLUX_URL, token: process.env.INFLUX_TOKEN });
    this.writeApi = this.influx.getWriteApi('org', 'iot_bucket');
    this.kafka = new Kafka({ brokers: ['kafka:9092'] }).producer(); // ← hardcode Kafka
  }
  async processTelemetry(sensorId, type, value, unit) {
    const point = new Point('telemetry').tag('sensor', sensorId).floatField(type, value);
    this.writeApi.writePoint(point);  // ← biết InfluxDB Point API
    // Đổi InfluxDB → TimescaleDB: phải sửa class business logic này
  }
}
```

**Tuân thủ trong IRMS** — `inventory-service/src/services/TelemetryProcessor.js`:

```javascript
// ✅ TelemetryProcessor chỉ biết abstraction — không biết infrastructure detail
const TelemetryRepository = require('../repositories/TelemetryRepository'); // abstraction: InfluxDB ở đây
const AlertPublisher      = require('./AlertPublisher');                     // abstraction: Kafka ở đây

class TelemetryProcessor {
  async processTelemetry(sensorId, type, value, unit) {
    // Persist — không biết dùng InfluxDB, TimescaleDB hay PostgreSQL
    await TelemetryRepository.saveTelemetry(sensorId, type, value, unit);
    // Evaluate thresholds — không biết alert đi đến Kafka, Slack hay email
...
Analytics Service áp dụng DIP theo hướng khác — **function injection**:

```javascript
// analytics-service/src/socket/dashboardSocket.js
function initSocket(server) {
  const io = socketIo(server);
  io.on('connection', (socket) => {
    buildInitialState(socket);   // push snapshot ngay khi Dashboard kết nối

    // ✅ DIP: inject broadcastMetricUpdate vào MetricsAggregator
    // Business logic (MetricsAggregator) không import Socket.io — infrastructure inject vào nó
    MetricsAggregator.setBroadcastFn(
      (metricName, payload) => io.emit('metricUpdate', { metricName, payload })
    );
  });
}

// analytics-service/src/services/MetricsAggregator.js
class MetricsAggregator {
  _broadcastFn = null;
  setBroadcastFn(fn) { this._broadcastFn = fn; }  // ← nhận dependency từ bên ngoài

  async processOrderPlaced(event) {
    await MetricsRepository.incrementOrderCount();
    // Không biết Socket.io tồn tại — chỉ biết "có một hàm broadcast nào đó"
    if (this._broadcastFn) this._broadcastFn('orderPlaced', { count: await MetricsRepository.getOrderCount() });
  }
}
```

**Phân tích**: Hai pattern DIP khác nhau cùng đạt mục tiêu: business logic không phụ thuộc infrastructure.  
- `TelemetryProcessor` dùng **module abstraction**: `TelemetryRepository` là module interface, implementation InfluxDB ẩn bên trong.  
- `MetricsAggregator` dùng **function injection**: `DashboardSocket` inject `broadcastFn` từ bên ngoài, `MetricsAggregator` không bao giờ `require('socket.io')`.  
Kết quả thực tế: test `TelemetryProcessor` với `InMemoryTelemetryRepository` không cần InfluxDB; test `MetricsAggregator` bằng cách inject mock `broadcastFn` không cần Socket.io server thật.
