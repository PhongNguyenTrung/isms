...
|-----|-----------|-------------------|-------------|-----|
| **S** | Single Responsibility | Mỗi module chỉ có một lý do để thay đổi | Robert C. Martin | 2003 |
| **O** | Open/Closed | Mở để mở rộng, đóng để chỉnh sửa | Bertrand Meyer | 1988 |
...
**Mối liên hệ giữa các nguyên lý**: SOLID không phải 5 nguyên lý rời rạc mà hỗ trợ lẫn nhau:
- **SRP** tạo ra các module nhỏ gọn → dễ áp dụng **OCP** (mở rộng không sửa).
- **LSP** đảm bảo **OCP** hoạt động đúng — nếu subclass vi phạm contract, mở rộng sẽ phá vỡ hệ thống.
- **ISP** giúp **DIP** khả thi — interface nhỏ gọn hơn thì dễ tạo abstraction hơn.
- **DIP** là nền tảng kỹ thuật cho **OCP** — inject dependency thay vì hardcode cho phép mở rộng linh hoạt.

---

#### 6.1.1. S — Single Responsibility Principle (SRP)

**Phát biểu gốc**: *"A module should be responsible to one, and only one, actor."*
...
**Bối cảnh**: OCP được Bertrand Meyer đề xuất lần đầu trong ngữ cảnh kế thừa (inheritance). Sau đó Robert C. Martin đã mở rộng ý nghĩa sang **Polymorphic OCP** — sử dụng abstraction (interface/abstract class) thay vì chỉ inheritance để đạt được OCP.
...
**Kết quả**: Thêm loại sensor mới (VD: `CO2Sensor`) chỉ implement `IReadableSensor`, không sửa interface hiện có.
...
| Nguyên lý | Lý thuyết (Mục 6) | Áp dụng trong IRMS (Mục 7) | Kết quả đo lường |
|-----------|-------------------|---------------------------|-----------------|
| **S** — SRP | Module 1 lý do thay đổi (§6.1.1) | Controller / Service / Repository tách biệt | < 10,000 LOC/service; unit test độc lập |
| **O** — OCP | Mở rộng không sửa code cũ (§6.1.2) | Strategy `NotificationChannel`; Kafka subscriber; `ALERT_ROUTING` config | Thêm kênh < 1 ngày, 0 regression |
| **L** — LSP | Swap không phá vỡ behavior (§6.1.3) | `InMemoryOrderRepository` ↔ `PostgresOrderRepository` | Test < 1ms, không cần DB thật |
| **I** — ISP | Client chỉ phụ thuộc interface cần (§6.1.4) | `IReadableSensor`, `IHealthCheckable`, `IRemoteConfigurable` | Thêm sensor không sửa interface cũ |
| **D** — DIP | Phụ thuộc abstraction (§6.1.5) | Constructor Injection qua DI Container | Test coverage 85%+, CI pipeline nhanh |
